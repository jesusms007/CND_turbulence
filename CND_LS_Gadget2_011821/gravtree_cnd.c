...
//External potential!!!!!!!!!!
    ...
    double R_cluster = 2.0 * pc_to_cm / All.UnitLength_in_cm ;/// code units
    double V0_cluster = 98.6 * 1.0e5 / All.UnitVelocity_in_cm_per_s ; /// code units
    double mbh_code = 4.0e6 *  SOLAR_MASS / All.UnitMass_in_g  ;  ///black hole mass in code units
    double r=0;
    double x=0,y=0,z=0;
    double alpha = 0;
    double cdisk = 0.71 ;
    double abar = 0.75 ; 
    double cbar = abar ;
    double Rdisk = 90.0 * pc_to_cm / All.UnitLength_in_cm ;/// code units  
    double vdisk = 190.0 * 1.0e5 / All.UnitVelocity_in_cm_per_s ; /// code units
    double beta_disk = 0.0;
    ...
    if(P[i].Ti_endstep == All.Ti_Current)
      {
          if (i == 100 && ThisTask == 0)
          {
              printf("--Calculating GC gravity NSC + BH + Disk/Bar (average) (Stolte+ 2008) --\n"); ///SALAS MOD
          }
          ...
          r = sqrt(x*x + y*y + z*z);
          if (r < 0.1)  /// less than 0.01 pc ///// 1 unit = 0.1 pc
          {
              r = sqrt( r*r + 0.1) ;
          }
              
          //BH contribution 4e6 Msun   
          P[i].GravAccel[0] += -All.G*mbh_code*x/r/r/r;
          P[i].GravAccel[1] += -All.G*mbh_code*y/r/r/r; // mass unit = 1e6 msun
          P[i].GravAccel[2] += -All.G*mbh_code*z/r/r/r;
          
          //Nuclear stellar cluster contribution
          alpha = (R_cluster*R_cluster) +  r*r ; /// code units
          P[i].GravAccel[0] += -V0_cluster*V0_cluster * x / alpha;
          P[i].GravAccel[1] += -V0_cluster*V0_cluster * y / alpha;
          P[i].GravAccel[2] += -V0_cluster*V0_cluster * z / alpha;

          ///Nuclear stellar disk contribution
          beta_disk = (Rdisk*Rdisk) + x*x + y*y + (z*z/(cdisk*cdisk) ) ;
          P[i].GravAccel[0] += -0.5 * vdisk*vdisk * x / beta_disk ;
          P[i].GravAccel[1] += -0.5 * vdisk*vdisk * y / beta_disk ;
          P[i].GravAccel[2] += -0.5 * vdisk*vdisk * (z/(cdisk*cdisk)) / beta_disk ;

          ///Nuclear bar contribution
          gamma_bar = (Rdisk*Rdisk) + (x*x/(abar*abar)) + y*y + (z*z/(cbar*cbar) ) ;
          P[i].GravAccel[0] += -0.5 * vdisk*vdisk * (x/(abar*abar)) / gamma_bar ;
          P[i].GravAccel[1] += -0.5 * vdisk*vdisk * y / gamma_bar ;
          P[i].GravAccel[2] += -0.5 * vdisk*vdisk * (z/(cbar*cbar)) / gamma_bar ;
