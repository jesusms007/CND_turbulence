...
///////SALAS PROTOTYPES///// MOD MARCH 21, 2019
///from read_turbulence.c
void read_turbulence(int fileturb, const char *normal_tur_file);
void create_phys_grid();
void grid_to_file_assignment();
///from ran3.c
float ran3(long *idum);
///from trilinear_box.c
int cell_1d(double coord);
int ind_below_box(double coord, int *true_cell, int opt);
int outside_box(double x, double y, double z);
int which_file(int number_of_files, double r);
int zero_or_one(double r);
double trilinear_box(double x, double y, double z, int v_dim, int write); ////v_dim is 0,1 or 2
