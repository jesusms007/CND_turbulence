...
void run(void)
{
  ...
  do				/* main loop */
      ...
      if((All.Time - All.TimeLastStatistics) >= All.TimeBetStatistics)
	{
      ...
      /*  if (All.Time == 0 )
        { //SALAS MOD. JUNE 2 2017 
            etot_sys = SysState.EnergyInt + SysState.EnergyPot + SysState.EnergyKin ; 
            omega_bar = ( (All.OmegaBar/kpc_to_km) * All.UnitTime_in_s ); /// code units ///JULY 3 MOD
            MPI_Bcast(&etot_sys, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
            MPI_Bcast(&omega_bar, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
            
            if(ThisTask == 0)
            { printf("\nTotal energy = %e\n", etot_sys*All.UnitEnergy_in_cgs);
              #ifdef ROTATING_BAR
                printf("\nOmega_bar = %e\n", (omega_bar/All.UnitTime_in_s)*kpc_to_km) ;
              #endif
            }

        }*/
    
    #ifdef DRIVING //SALAS MOD. FEB 22 2017 
        if (All.Time != All.TimeBegin || All.Time != 0.0)
        {
            if(ThisTask == 0)
            {
                printf("****Changing file assigment to each grid***\n");
                grid_to_file_assignment();
                printf("****Changed file assigment to each grid-DONE***\n");
            }
            #ifdef SN_ONE_BOX
            double sn_random1,sn_random2  ;
            sn_random1 = gsl_rng_uniform(random_generator) ;  ///random number from 0 to 1
            sn_random2 = gsl_rng_uniform(random_generator) ;
            SN_BOX1 = zero_or_one( sn_random1); ///random number from 0 OR 1
            SN_BOX2 = zero_or_one( sn_random2);
            //SN_BOX3 = zero_or_one( sn_random);
