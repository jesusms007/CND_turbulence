...
#include "allvars.h"
#include "proto.h"
//#include "Salas_addition/cooling_clouds2.h"
 ...
 */
void   advance_and_find_timesteps()
{
  ...
  ///////////////TURB. DRIVING///////////SALAS MOD. MARCH 3 2017 
#ifdef DRIVING 
            if (DriveFlag==1)
            {
                //if ( (ThisTask == 0) && (i == 10) )
               // {
               //     printf("Particle velocity before = %e, %e, %e \n", P[10].Vel[0], P[10].Vel[1], P[10].Vel[2]);
               // }
                
                if (Alternating == 0)
                {
                    P[i].Vel[0] += P[i].TURB_VEL[0];
                    P[i].Vel[1] += P[i].TURB_VEL[1];
                    P[i].Vel[2] += P[i].TURB_VEL[2];
                }
                if (Alternating == 1)
                {
                    P[i].Vel[0] -= P[i].TURB_VEL[0];
             ...
             //   if ( (ThisTask == 0) && (i == 10) )
             //   {
              //      printf("Turbulence added = %e, %e, %e \n", P[i].TURB_VEL[0], P[i].TURB_VEL[1], P[i].TURB_VEL[2]);
             //       printf("Particle velocity after = %e, %e, %e \n", P[10].Vel[0], P[10].Vel[1], P[10].Vel[2]);
             //   }
 ...
 ////////////////////////////////////////////////////////////////
 ////////////////////////////////////////////////////////////////
#ifdef MYWIND  ///SALAS MOD
    double x = P[i].Pos[0];
    double y = P[i].Pos[1];
    double z = P[i].Pos[2];
    double rsph = sqrt( x*x + y*y + z*z) ;
    double kms = All.UnitVelocity_in_cm_per_s / 1.0e5 ;
    double phi_bh = -4.301e-3 * 4.0e6 / (rsph*unit_lenght_in_pc) ;
    double phi_cl = 0.5 * (98.6)*(98.6) * log(4.0 + pow( (rsph*unit_lenght_in_pc), 2.0) ) ;
    double phi = phi_bh + phi_cl ; /// (km/s)^2
    double the_vels; /// = sqrt( 2.0* fabs(phi) ) / kms;
    double the_r_min = 0.5 / ((double) unit_lenght_in_pc) ;
    double the_r_max = 1.5 / ((double) unit_lenght_in_pc) ;
    double the_rho = 100.0 * 2.0 * PROTONMASS / All.UnitDensity_in_cgs ;
    if (SphP[i].Density >= the_rho)
    {
        the_vels = (sqrt( 2.0* fabs(phi) ) / kms) * sqrt( the_rho/ SphP[i].Density);
    }
    if (SphP[i].Density < the_rho)
    {
        the_vels = (sqrt( 2.0* fabs(phi) ) / kms) ;
    }
    double fvs = 0.05 ;   
    if (rsph > the_r_min && rsph < the_r_max)
    {
        P[i].Vel[0] += fvs * the_vels * x/rsph;
        P[i].Vel[1] += fvs * the_vels * y/rsph;
        P[i].Vel[2] += fvs * the_vels * z/rsph;
    }
#endif
 ////////////////////////////////////////////////////////////////
 ////////////////////////////////////////////////////////////////
//////////////COOLING/////////////////// Edit 9/5/16
    ...
    double unit_internal_energy = pow(All.UnitVelocity_in_cm_per_s , 2.0);
      double unit_ergs = unit_internal_energy * All.UnitMass_in_g ;
   
     // SphP[i].Entropy += (  dtA_coolheat_c(SphP[i].Entropy, SphP[i].DtEntropy, dt_entr, SphP[i].Density,
       //         All.UnitMass_in_g, All.UnitTime_in_s,  unit_ergs, All.UnitLength_in_cm, unit_internal_energy) ) ;
      
   SphP[i].Entropy =  gas_cooling_entropy(SphP[i].Entropy, SphP[i].DtEntropy, dt_entr, SphP[i].Density, All.UnitMass_in_g,
                        All.UnitTime_in_s,unit_ergs,  All.UnitLength_in_cm,unit_internal_energy, All.UnitDensity_in_cgs) ;
