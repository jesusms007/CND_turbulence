...
int ind_below_box(double coord, int *true_cell, int opt)
{
    ...
    if (opt == 2)
    {
        zin = (fabs( (galactic_grid_z.grid[0] - coord) / delta2) ) ;
    }
    else // opt == 1 or 2
    {
...
int outside_box(double x, double y, double z)
{
    ...
    double limit_low_z=0; double limit_high_z=0;
    int c=-1;
    ...
    limit_low_z = galactic_grid_z.grid[0];
    limit_high_z = galactic_grid_z.grid[grid_size-1];
   // printf("limits= %f, %f\n", limit_low, limit_high);
    ...
    else if ( (z <= limit_low_z) || (z >= limit_high_z) )
    {
        c = 0;
    }
    else ///inside box
...
int zero_or_one(double r) ///RETURNS A RANDOM NUMBER, 1 or 2
{
    int result = 0 ;
    if (r <= 0.5)
    {
...
double trilinear_box(double x, double y, double z, int v_dim, int write) ////v_dim is 0,1 or 2 for x, y ,z
{
    ...
    double c = 0.0;
    ///indices
    int index_x0 = ind_below_box( x , &which_cell_x, 0 );
    int index_x1 = index_x0 + 1;
    int index_y0 = ind_below_box( y , &which_cell_y, 1 );
    int index_y1 = index_y0 + 1;
    int index_z0 = ind_below_box( z , &which_cell_z, 2 );
    int index_z1 = index_z0 + 1;
    
    #ifdef SN_ONE_BOX
    if ( which_cell_x == SN_BOX1 && which_cell_y == SN_BOX2 )
    {
        check_flag = 0 ;
    }
    else
    ...
    if ( (index_x0 == -1) || (index_y0 == -1) || (index_z0 == -1) )
    {
    ...
    {
        c = 0.0;
        //printf("My_error, check_flag = %d\n", check_flag);
       // exit(0);
    }
    else if (check_flag == 0) /// if no errors
    {
        ///values
         ...
         */
        	if (    (which_cell_x > number_of_grids) || (which_cell_x < 0)
                ...
                ||  (which_cell_z != 0) ) /// cell is the grid
        	{
            ...
            z0 = galactic_grid_z.grid[index_z0];
        
        int file = file_of_each_grid[which_cell_x*number_of_grids +  which_cell_y] ;
...
{ 
  ...
  //  #else
   ...
   // double local_vx =0; double local_vy = 0; double local_vz=0;
    ...
    for (count_quad=0; count_quad<NumPart; count_quad++)
    {
      ...
      // int whatfile = which_file( Nturbfiles, myran1);
        ...
        if ( P[count_quad].TURB_VEL[0] == 0 && P[count_quad].TURB_VEL[1] == 0 && P[count_quad].TURB_VEL[2] == 0)
        {
            zero_count_local++;
        }
	  ...
	  scaling = pow(  SphP[count_quad].Density , -1.0/4.0) ; 
	  //  else
            //scaling =1.0;
        }
   ...
   // #endif
    ...
    MPI_Allreduce(&zero_count_local, &zero_count_global, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    ...
    for (lll=0;lll<NumPart;lll++)
    {
	  ...
	  scaling_again = pow( SphP[lll].Density , -1.0/4.0 ) ; 
        }
